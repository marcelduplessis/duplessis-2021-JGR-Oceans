def process_slocum_data_L1(dat):
    ...
    dat.salinity[dat.salinity<33.8] = np.NaN
    dat.temperature[dat.salinity<33.8] = np.NaN
    
    # calculate the conservative temperature and absolute salinity
    dat['salinity']    = (('time'), gsw.SA_from_SP(dat.salinity, dat.ctd_pressure, dat.longitude, dat.latitude))
    dat['temperature'] = (('time'), gsw.CT_from_t( dat.salinity ,dat.temperature , dat.ctd_pressure))
    ...
    for i, val in enumerate(dives):
        ...
        temp[:,i] = gt.cleaning.rolling_window(griddata(x, t, depth, method='linear'), func=np.nanmean, window=5)
        salt[:,i] = gt.cleaning.rolling_window(griddata(x, s, depth, method='linear'), func=np.nanmean, window=5)
